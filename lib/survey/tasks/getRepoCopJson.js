...
function getRepoCopJson(organisation, next) {
    ...
    var q = async.queue(function(task, callback) {
        logger.info("Fetching %s metadata", task.repo.full_name, task.organisation.providerKey);
    ...
    function parseRepoCopJson(repo, buffer, next) {
        var meta = buffer.toString('utf8');
        json.parse(meta, function(err, repocopJson) {
            if (err) logger.warn('missing or cannot parse repocop.json', repo.name, err.message);
            else repo.metadata = repocopJson;
            next();                
        });
    }
