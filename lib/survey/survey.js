...
function display(req, res) {
    load(function(err, repositories) {
        ...
        if (login) repositories = getUserActivity(login, repositories);
...
function getUserActivity(login, repositories) {
    var userRepos = [];
    repositories.forEach(function (repo) {
        if (!(repo.activity && repo.activity.length)) return;
        repo.activity = repo.activity.filter(function (act) {
            return act.login == login;
        });
        if (repo.activity.length) userRepos.push(repo);
    });
    return userRepos;
}
